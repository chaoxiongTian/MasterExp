...
def save_bigest_5_segmented(image, segmented_pixel_list, sava_folder, flag):
    ...
    """
    (image_w, image_h) = image.size
    segmented_pixel_list = sorted(
        segmented_pixel_list, key=lambda x: len(x), reverse=True)[0:flag]
    iamge_new = Image.new('RGB', (image_w, image_h), (255, 255, 255))
